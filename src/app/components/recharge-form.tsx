...
export default function RechargeForm() {
  ...
  async function handleRecharge(data: any) {
    const { walletRecharge, payment } = await recharge({
      requestedAmount: data.requestedAmount,
    })
    reset()
    toast.success('Requisição de recarga efetuada com sucesso!')
    setTimeout(() => router.push(`/recharge/${payment.id}/payment`), 1500)
  }
        ...
        <fieldset>
          <div>
            <div className="px-4 py-2">
              <label htmlFor="requestedAmount" className="text-3xl font-bold">
